...
const DashGestor = () => {
  ...
  const processTerritoryData = useCallback((data) => {
    ...
    const territoryCounts = filteredData.reduce((acc, visit) => {
      const cod = visit.visit_cod;
      const pessoas = Number(visit.num_pessoas) || 0;

      if (!acc[cod]) {
        acc[cod] = { visitas: 0, pessoas: 0 };
      }

      acc[cod].visitas += 1;
      acc[cod].pessoas += pessoas;
  ...
  const processStatusData = useCallback((territories) => {
    ...
    const totalSurdos = territories.reduce((sum, territory) => {
      const numPessoas = Number(territory.num_pessoas);
      return sum + (isNaN(numPessoas) ? 0 : numPessoas);
    }, 0);
    ...
    setStatusSummary({
      ...
      totalSurdos, // aqui é o novo campo!
    });
  }, []);
  ...
  const processChartData = useCallback(() => {
    ...
    filteredData.forEach((visit) => {
      ...
      // Soma de surdos (num_pessoas)
      if (visit.num_pessoas !== undefined && visit.num_pessoas !== null) {
        totalSurdos += Number(visit.num_pessoas) || 0;
      }
    ...
    setSummary({
      ...
      totalSurdos, // soma total de surdos
    });
        ...
        >
          ...
          <Box sx={{ marginTop: "20px" }}>
            <Typography variant="body1">
              Total de Endereços: <strong>{statusSummary.totalColors}</strong>
              (<strong>{statusSummary.totalSurdos}</strong> Surdos)
            </Typography>
            <Typography variant="body1">
