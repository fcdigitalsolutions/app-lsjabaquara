...
const IndicaForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editRow) {
      try {
        await axios.put(`http://127.0.0.1:5000/indica/${editRow.id}`, { nome_publica, enderec, end_confirm, obs });
      ...
      try {
        const response = await axios.post('http://127.0.0.1:5000/indica', { data_inclu, nome_publica, cod_congreg, cod_regiao, enderec, end_confirm, origem, obs });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:5000/indica/${id}`);
